...
TypeScript entrypoints `src/evm.ts` and `src/starknet.ts` orchestrate network-specific processors from `src/evm` and `src/starknet`, with shared utilities in `src/_shared` and runtime logic in `src/runtime.ts`. The independently scheduled token-price process lives in `src/price-sync`. Configuration bootstraps in `src/config.ts`, which layers `.env` files per network. SQL migrations under `migrations/` execute sequentially through `scripts/migrate.ts`. Tests live next to focused source modules or in `tests/migrations/*.test.ts`, reusing the harness in `tests/helpers` when a database is needed. One-off operational scripts sit in `scripts/`. The production App Platform definition is tracked in `.do/app.yaml`, and automated Postgres dumps are handled via `.github/workflows/pg-dump.yaml`.
