...
export class DAO {
  ...
  private async createSchema(): Promise<void> {
    await this.pg.query(`
        ...
        WITH all_tokens AS (SELECT token0 AS token FROM pool_keys UNION DISTINCT SELECT token1 FROM pool_keys),
             ...
             pair_swap_counts_by_day AS (SELECT date_bin(INTERVAL '1 day', b.time, '2000-01-01') AS day,
                                                pk.token0,
                                                pk.token1,
  ...
  public async batchInsertFakeFeeEvents(
    ...
    await this.pg.query({
      text: `
          INSERT INTO ${tableName} (event_id, pool_key_hash, owner, salt, lower_bound, upper_bound, delta0, delta1)
          SELECT *
          FROM UNNEST($1::BIGINT[], $2::NUMERIC[], $3::NUMERIC[], $4::NUMERIC[], $5::INT[], $6::INT[], $7::NUMERIC[], $8::NUMERIC[])
      `,
      values: [
