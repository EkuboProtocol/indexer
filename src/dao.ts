...
export class DAO {
  ...
  public async refreshAnalyticalTables({ since }: { since: Date }) {
    // todo: turn these analytical views into trigger-updated tables
    // await this.pg.query({
    //   text: `
    //             WITH swap_data AS (
    //                 SELECT pbc.pool_key_id                                                      AS   pool_key_id,
    ...
    //                          JOIN pool_keys ON swaps.chain_id = pool_keys.chain_id AND pbc.pool_key_id = pool_keys.id
    //                          JOIN event_keys ON swaps.chain_id = event_keys.chain_id AND swaps.pool_balance_change_id = event_keys.sort_id
    ...
    //                 GROUP BY hour, pbc.pool_key_id, token
    //             ),
    //             fees_token0 AS (
    //                 SELECT pbc.pool_key_id                AS pool_key_id,
    //                        DATE_TRUNC('hour', blocks.time) AS hour,
    ...
    //                          JOIN pool_keys ON fa.chain_id = pool_keys.chain_id AND pbc.pool_key_id = pool_keys.id
    //                          JOIN event_keys ON fa.chain_id = event_keys.chain_id AND pbc.event_id = event_keys.sort_id
    //                          JOIN blocks ON event_keys.chain_id = blocks.chain_id AND event_keys.block_number = blocks.number
    ...
    //                 (WITH rev0 AS (SELECT pbc.pool_key_id                AS pool_key_id,
    //                                       DATE_TRUNC('hour', blocks.time) AS hour,
    //                                       pk.token0                          token,
    ...
    //                                         JOIN pool_keys pk ON pu.chain_id = pk.chain_id AND pbc.pool_key_id = pk.id
    //                                         JOIN event_keys ek ON pu.chain_id = ek.chain_id AND pbc.event_id = ek.sort_id
    ...
    //                                  AND pk.fee != 0
    //                                GROUP BY hour, pbc.pool_key_id, token),
    //                       rev1 AS (SELECT pbc.pool_key_id                AS pool_key_id,
    //                                       DATE_TRUNC('hour', blocks.time) AS hour,
    //                                       pk.token1                          token,
    ...
    // await this.pg.query({
    //   text: `
    //             WITH total_swaps_per_block_pair AS (SELECT s.chain_id,
    //                                                        pk.token0        AS token0,
    //                                                        pk.token1        AS token1,
    ...
    //                                                          JOIN event_keys ek ON pbc.event_id = ek.sort_id
    //                                                          JOIN pool_keys pk ON pbc.pool_key_id = pk.id
    //                                                 WHERE s.chain_id = $1
    //                                                 GROUP BY s.chain_id, block_number, pk.token0, pk.token1)
    //             INSERT
    ...
    //                           JOIN blocks b ON tspt.chain_id = b.chain_id AND tspt.block_number = b.number
    //                  WHERE total_delta0 != 0
    //                    AND total_delta1 != 0
    //                    AND DATE_TRUNC('hour', b.time) >= DATE_TRUNC('hour', $2::timestamptz)
    ...
    //                               swap_count = excluded.swap_count;
    //         `,
    //   values: [this.chainId, since],
    // });

    // await this.pg.query({
    //   text: `
    //     INSERT INTO hourly_tvl_delta_by_token
    //         (WITH first_event_id AS (SELECT sort_id
    //                                   FROM event_keys AS ek
    //                                           JOIN blocks AS b ON ek.block_number = b.number
    //                                   WHERE chain_id = $1 AND b.time >= DATE_TRUNC('hour', $1::timestamptz)
    //                                   ORDER BY sort_id
    //                                   LIMIT 1),
    ...
    //                       DATE_TRUNC('hour', blocks.time) AS hour,
    //                       CASE
    //                           WHEN pu.liquidity_delta IS NOT NULL AND pu.liquidity_delta < 0 THEN
    ...
    //                     LEFT JOIN position_updates pu ON pbc.event_id = pbc.event_id
    //                   WHERE pbc.event_id >= (SELECT sort_id FROM first_event_id)
    //               ),
