...
export class DAO {
  ...
  private async createSchema(): Promise<void> {
    await this.pg.query(`
        ...
        CREATE OR REPLACE VIEW hourly_ohlc_data_view AS (
          WITH last_swap_per_block_per_pair AS (
            SELECT
              b.time,
              pk.token0,
              pk.token1,
              max(event_id) last_event_id
            FROM
              swaps s
            ...
            prices AS (
              SELECT
                date_trunc('hour', lspbpp.TIME) AS HOUR,
              lspbpp.time,
              token0,
              token1,
              s.tick_after AS tick
            FROM
              last_swap_per_block_per_pair lspbpp
              JOIN swaps s ON lspbpp.last_event_id = s.event_id),
            prices_with_context AS (
              SELECT
                hour,
                p.TIME,
                p.token0,
                p.token1,
                p.tick,
                lag(p.tick) OVER (PARTITION BY p.token0,
                  p.token1 ORDER BY p.time) AS last_tick,
              lag(p.time) OVER (PARTITION BY p.token0,
                p.token1 ORDER BY p.time) AS last_time,
              lead(p.time) OVER (PARTITION BY p.token0,
                p.token1 ORDER BY p.time) AS next_time,
              lead(p.tick) OVER (PARTITION BY p.token0,
                p.token1 ORDER BY p.time) AS next_tick,
              row_number() OVER (PARTITION BY hour,
                p.token0,
                p.token1 ORDER BY p.time) AS row_no,
              row_number() OVER (PARTITION BY hour,
                p.token0,
                p.token1 ORDER BY p.time DESC) AS row_no_reverse
            FROM
              prices p),
            open_prices AS (
              SELECT
                  ...
                  low_prices AS (
                    ...
                    tick_cumulatives AS (
                      SELECT
                        ...
                        p.token1,
                        p.hour,
                        sum(p.last_tick * extract(epoch FROM p.time - GREATEST (p.last_time, p.hour))) AS tick_cumulative,
                        sum(extract(epoch FROM p.time - GREATEST (p.last_time, p.hour))) AS seconds_in_period
                    FROM
                      prices_with_context p
                    GROUP BY
                      p.token0,
                      p.token1,
                      p.hour
                    ...
                    SELECT
                      ...
                      lp.tick AS low,
                      cp.tick AS CLOSE,
                      floor(tc.tick_cumulative / tc.seconds_in_period)::int4 AS average
                    FROM
                      open_prices op
                      JOIN close_prices cp ON op.hour = cp.hour
                        AND op.token0 = cp.token0
                        AND op.token1 = cp.token1
                      JOIN high_prices hp ON op.hour = hp.hour
                        AND op.token0 = hp.token0
                        AND op.token1 = hp.token1
                      JOIN low_prices lp ON op.hour = lp.hour
                        AND op.token0 = lp.token0
                        AND op.token1 = lp.token1
                      JOIN tick_cumulatives tc ON tc.hour = op.hour
                        AND tc.token0 = op.token0
                        AND tc.token1 = op.token1);

                        
        CREATE MATERIALIZED VIEW IF NOT EXISTS hourly_ohlc_data_materialized AS
        SELECT * FROM hourly_ohlc_data_view;

        CREATE UNIQUE INDEX IF NOT EXISTS idx_hourly_ohlc_data_materialized
            ON hourly_ohlc_data_materialized (token0, token1, hour);
    `);
  }
  ...
  public async refreshAnalyticalTables({ since }: { since: Date }) {
    ...
    await this.pg.query(`
      ...
      REFRESH MATERIALIZED VIEW CONCURRENTLY hourly_ohlc_data_materialized;
    `);
  }
