...
CREATE OR REPLACE FUNCTION apply_hourly_volume_delta(
        p_pool_key_id bigint,
        p_hour timestamptz,
        p_token NUMERIC,
        p_volume NUMERIC,
        p_fees NUMERIC,
        p_swap_count NUMERIC,
        p_allow_insert BOOLEAN
    ) RETURNS void AS $$ BEGIN IF p_volume = 0
    AND p_fees = 0
    AND p_swap_count = 0 THEN RETURN;
END IF;
UPDATE hourly_volume_by_token
...
WHERE pool_key_id = p_pool_key_id
    AND hour = p_hour
    AND token = p_token;
IF NOT FOUND THEN IF NOT p_allow_insert THEN RETURN;
END IF;
...
VALUES (
        ...
        p_fees,
        p_swap_count
    ) ON CONFLICT (pool_key_id, hour, token) DO
UPDATE
...
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION apply_hourly_volume_from_fees_accumulated(
        p_chain_id bigint,
        p_pool_balance_change_id bigint,
        p_multiplier NUMERIC
    ) RETURNS void AS $$
...
IF NOT FOUND THEN RETURN;
END IF;
IF v_fee0 <> 0 THEN PERFORM apply_hourly_volume_delta(
    v_pool_key_id,
    v_hour,
    v_token0,
    0,
...
END IF;
IF v_fee1 <> 0 THEN PERFORM apply_hourly_volume_delta(
    v_pool_key_id,
    v_hour,
    v_token1,
    0,
...
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION apply_hourly_price_delta(
        p_chain_id bigint,
        p_token0 NUMERIC,
        p_token1 NUMERIC,
        p_hour timestamptz,
        ...
        p_swap_count NUMERIC,
        p_allow_insert BOOLEAN
    ) RETURNS void AS $$ BEGIN IF p_k_volume = 0
    AND p_total = 0
    AND p_swap_count = 0 THEN RETURN;
END IF;
UPDATE hourly_price_data
SET k_volume = k_volume + p_k_volume,
    total = total + p_total,
    swap_count = swap_count + p_swap_count
...
VALUES (
        ...
        p_total,
        p_swap_count
    ) ON CONFLICT (chain_id, token0, token1, hour) DO
UPDATE
...
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION apply_hourly_price_from_swap(
        p_chain_id bigint,
        p_pool_balance_change_id bigint,
        p_multiplier NUMERIC
    ) RETURNS void AS $$
DECLARE v_token0 NUMERIC;
v_token1 NUMERIC;
v_block_number int8;
v_hour timestamptz;
v_delta0 NUMERIC;
...
SELECT total_delta0,
    ...
    v_prev_total_delta1,
    v_prev_swap_count
FROM hourly_price_block_totals
WHERE chain_id = p_chain_id
    AND token0 = v_token0
    AND token1 = v_token1
    AND block_number = v_block_number;
IF NOT FOUND THEN v_prev_total_delta0 := 0;
...
IF v_new_total_delta0 = 0
AND v_new_total_delta1 = 0
AND v_new_swap_count = 0 THEN
...
WHERE chain_id = p_chain_id
    ...
    AND block_number = v_block_number;
ELSE
INSERT INTO hourly_price_block_totals (
        chain_id,
        token0,
        token1,
        block_number,
...
VALUES (
        ...
        v_new_total_delta1,
        v_new_swap_count
    ) ON CONFLICT (chain_id, token0, token1, block_number) DO
UPDATE
SET total_delta0 = EXCLUDED.total_delta0,
    total_delta1 = EXCLUDED.total_delta1,
    swap_count = EXCLUDED.swap_count,
    hour = EXCLUDED.hour;
END IF;
IF v_prev_total_delta0 <> 0
AND v_prev_total_delta1 <> 0 THEN v_prev_k_volume := ABS(v_prev_total_delta0 * v_prev_total_delta1);
v_prev_total := v_prev_total_delta1 * v_prev_total_delta1;
v_prev_swap_contrib := v_prev_swap_count;
ELSE v_prev_k_volume := 0;
v_prev_total := 0;
v_prev_swap_contrib := 0;
END IF;
IF v_new_total_delta0 <> 0
AND v_new_total_delta1 <> 0 THEN v_new_k_volume := ABS(v_new_total_delta0 * v_new_total_delta1);
v_new_total := v_new_total_delta1 * v_new_total_delta1;
v_new_swap_contrib := v_new_swap_count;
ELSE v_new_k_volume := 0;
v_new_total := 0;
...
PERFORM apply_hourly_price_delta(
    p_chain_id,
    v_token0,
    v_token1,
    v_hour,
...
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION apply_hourly_tvl_delta(
        p_pool_key_id bigint,
        p_hour timestamptz,
        p_token NUMERIC,
        p_delta NUMERIC,
        p_allow_insert BOOLEAN
    ) RETURNS void AS $$ BEGIN IF p_delta = 0 THEN RETURN;
END IF;
UPDATE hourly_tvl_delta_by_token
SET delta = delta + p_delta
WHERE pool_key_id = p_pool_key_id
    AND hour = p_hour
    AND token = p_token;
IF NOT FOUND THEN IF NOT p_allow_insert THEN RETURN;
...
VALUES (p_pool_key_id, p_hour, p_token, p_delta) ON CONFLICT (pool_key_id, hour, token) DO
UPDATE
...
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION apply_hourly_revenue_delta(
        p_pool_key_id bigint,
        p_hour timestamptz,
        p_token NUMERIC,
        p_revenue NUMERIC,
        p_allow_insert BOOLEAN
    ) RETURNS void AS $$ BEGIN IF p_revenue = 0 THEN RETURN;
END IF;
UPDATE hourly_revenue_by_token
...
VALUES (p_pool_key_id, p_hour, p_token, p_revenue) ON CONFLICT (pool_key_id, hour, token) DO
UPDATE
...
END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION maintain_hourly_tvl_delta_from_pool_balance_change() RETURNS TRIGGER AS $$ BEGIN IF TG_OP = 'INSERT' THEN PERFORM apply_hourly_tvl_from_pool_balance_change(NEW.chain_id, NEW.event_id, 1);
ELSIF TG_OP = 'UPDATE' THEN PERFORM apply_hourly_tvl_from_pool_balance_change(OLD.chain_id, OLD.event_id, -1);
PERFORM apply_hourly_tvl_from_pool_balance_change(NEW.chain_id, NEW.event_id, 1);
ELSIF TG_OP = 'DELETE' THEN PERFORM apply_hourly_tvl_from_pool_balance_change(OLD.chain_id, OLD.event_id, -1);
END IF;
RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
INSERT
    OR
UPDATE
    OR DELETE ON swaps FOR EACH ROW EXECUTE FUNCTION maintain_hourly_metrics_from_swaps();
CREATE TRIGGER maintain_hourly_volume_from_fees_accumulated
AFTER
INSERT
    OR
UPDATE
    OR DELETE ON fees_accumulated FOR EACH ROW EXECUTE FUNCTION maintain_hourly_volume_from_fees_accumulated();
CREATE CONSTRAINT TRIGGER maintain_hourly_tvl_delta_from_pool_balance_change
AFTER
INSERT
    OR
UPDATE
    OR DELETE ON pool_balance_change DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION maintain_hourly_tvl_delta_from_pool_balance_change();
CREATE TRIGGER maintain_hourly_revenue_from_position_updates
AFTER
INSERT
    OR
UPDATE
    OR DELETE ON position_updates FOR EACH ROW EXECUTE FUNCTION maintain_hourly_revenue_from_position_updates();