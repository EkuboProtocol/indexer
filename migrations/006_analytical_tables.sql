...
BEGIN
    ...
    fees_token0 AS (
        ...
        WHERE fa.chain_id = p_chain_id
            ...
            AND pbc.delta0 > 0
            AND DATE_TRUNC('hour', b.time) = v_hour
        GROUP BY pbc.pool_key_id, pk.token0
    ),
    fees_token1 AS (
        SELECT
            pbc.pool_key_id,
        ...
        WHERE fa.chain_id = p_chain_id
            ...
            AND pbc.delta1 > 0
            AND DATE_TRUNC('hour', b.time) = v_hour
        GROUP BY pbc.pool_key_id, pk.token1
    ),
    combined_data AS (
        SELECT * FROM swap_data
        UNION ALL
        SELECT * FROM fees_token0
        UNION ALL
        SELECT * FROM fees_token1
    )
    SELECT
        pool_key_id,
        v_hour AS hour,
...
BEGIN
    ...
    WITH adjusted_pool_balance_changes AS (
        SELECT
            pbc.pool_key_id,
            v_hour AS hour,
            CASE
                WHEN pu.liquidity_delta IS NOT NULL AND pu.liquidity_delta < 0 THEN
                    CEIL(
                        (pbc.delta0 * 18446744073709551616::NUMERIC) /
                        (18446744073709551616::NUMERIC - pk.fee)
                    )
                ELSE pbc.delta0
            END AS delta0,
            CASE
                WHEN pu.liquidity_delta IS NOT NULL AND pu.liquidity_delta < 0 THEN
                    CEIL(
                        (pbc.delta1 * 18446744073709551616::NUMERIC) /
                        (18446744073709551616::NUMERIC - pk.fee)
                    )
                ELSE pbc.delta1
            END AS delta1
...
BEGIN
    ...
    SELECT
        ...
        pk.token0,
        pk.token1,
        DATE_TRUNC('hour', b.time)
    INTO v_pool_key_id, v_token0, v_token1, v_hour
    FROM pool_balance_change pbc
        JOIN pool_keys pk ON pk.id = pbc.pool_key_id
        JOIN event_keys ek ON ek.chain_id = pbc.chain_id
            AND ek.sort_id = pbc.event_id
...
FOR EACH ROW EXECUTE FUNCTION maintain_hourly_metrics_from_swaps();

CREATE TRIGGER maintain_hourly_volume_from_fees_accumulated
AFTER INSERT OR UPDATE ON fees_accumulated
FOR EACH ROW EXECUTE FUNCTION maintain_hourly_volume_from_fees_accumulated();

CREATE CONSTRAINT TRIGGER maintain_hourly_tvl_delta_from_pool_balance_change
AFTER INSERT OR UPDATE ON pool_balance_change
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION maintain_hourly_tvl_delta_from_pool_balance_change();

CREATE TRIGGER maintain_hourly_revenue_from_position_updates
AFTER INSERT OR UPDATE ON position_updates
FOR EACH ROW EXECUTE FUNCTION maintain_hourly_revenue_from_position_updates();
