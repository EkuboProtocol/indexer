...
-- The operator reset has to clear the head too. It rewinds the cursor and
-- deletes everything above the target height, and after that the recorded
-- head is a block on the part of the chain just discarded. The indexer's own
-- rewind paths clear it the same way (writeCursor without a head); this keeps
-- the manual path consistent, so a reader sees nothing rather than an orphaned
-- height until the next block is processed.
--
-- Note the existing unique_key lookup reads blocks at the target height. That
-- height should be a block that carried events; an empty one has no blocks
-- row now, so unique_key comes back NULL -- which is what already happened
-- for any empty block older than a day.
