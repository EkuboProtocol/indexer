...
CREATE MATERIALIZED VIEW spline_pools_materialized AS (
    SELECT DISTINCT pool_key_id
    FROM spline_liquidity_updated
);
CREATE UNIQUE INDEX idx_spline_pools_materialized_pool_key_hash ON spline_pools_materialized USING btree (pool_key_id);