	...
	WITH proposal_times AS (
		...
		FROM
			...
			JOIN LATERAL ( WITH events AS (
					...
					WHERE
						bl.time BETWEEN pt.proposal_time AND pt.vote_start
					UNION ALL
					SELECT
						w.delegate, bl.time, - w.amount AS delta
					FROM
						staker_withdrawn w
					...
					SELECT
						s2.delegate, pt.proposal_time AS time,
					...
					WHERE
						bl2.time < pt.proposal_time
					GROUP BY
						s2.delegate
					UNION ALL
					SELECT
						w2.delegate, pt.proposal_time AS time,
					...
					WHERE
						bl3.time < pt.proposal_time
					GROUP BY
						w2.delegate
					UNION ALL
					-- sentinel at vote_start to cap last interval
					SELECT
						d.delegate, pt.vote_start AS time,
						0::numeric AS delta
					FROM (
						SELECT
							delegate
					...
					stake_running AS (
						SELECT
							delegate,
							time,
							sum(delta) OVER (PARTITION BY delegate ORDER BY time ROWS UNBOUNDED PRECEDING) AS stake_amount
					FROM
						events),
					...
					intervals AS (
						SELECT
							delegate,
							time AS start_time,
							lead(time) OVER (PARTITION BY delegate ORDER BY time) AS end_time,
						stake_amount
					FROM
						stake_running) -- integrate stake_amount * duration
					SELECT
						delegate,
						sum(stake_amount * extract(EPOCH FROM (end_time - start_time))) AS weighted_time_sum
					FROM
						intervals
					WHERE
...
CREATE MATERIALIZED VIEW IF NOT EXISTS proposal_delegate_voting_weights_materialized AS (
	SELECT
		...
		voting_weight
	FROM
		proposal_delegate_voting_weights_view);
