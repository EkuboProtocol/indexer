CREATE TABLE position_current_liquidity (
    ...
    PRIMARY KEY (chain_id, pool_key_id, locker, salt, lower_bound, upper_bound)
);
...
BEGIN
    ...
    ON CONFLICT (chain_id, pool_key_id, locker, salt, lower_bound, upper_bound)
    DO UPDATE SET
...
BEGIN
    ...
    WHERE chain_id = OLD.chain_id
      AND pool_key_id = OLD.pool_key_id
