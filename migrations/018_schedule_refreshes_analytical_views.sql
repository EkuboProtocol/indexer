...
SELECT
	cron.schedule ('refresh_last_24h_pool_stats_materialized', '0 * * * *', $$
		SELECT
...
SELECT
	cron.schedule ('refresh_token_pair_realized_volatility', '*/5 * * * *', $$
		SELECT
			safe_refresh_mv ('token_pair_realized_volatility');

$$);

SELECT
	cron.schedule ('refresh_pool_market_depth', '*/5 * * * *', $$
		SELECT
			safe_refresh_mv ('pool_market_depth');

$$);

SELECT
	cron.schedule ('refresh_proposal_delegate_voting_weights_materialized', '*/5 * * * *', $$
		SELECT
...
SELECT
	cron.schedule ('refresh_latest_token_registrations', '*/5 * * * *', $$
		SELECT
			safe_refresh_mv ('latest_token_registrations');
