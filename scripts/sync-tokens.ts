...
async function main() {
  await sql.begin(async (sql) => {
    ...
    const { count: ethMainnetTokensInserted } = await sql`
    ...
    const { count: ethSepoliaTokensInserted } = await sql`
        INSERT INTO erc20_tokens ${sql(
    ...
    const { count: starknetMainnetTokensInserted } = await sql`
        INSERT INTO erc20_tokens ${sql(
    ...
    const { count: starknetSepoliaTokensInserted } = await sql`
        INSERT INTO erc20_tokens ${sql(
